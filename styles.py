...
def textbox(*textboxes):
    for t in textboxes:
        t.setStyleSheet(
            'QLineEdit{padding:2,2,1,12;border:1px solid silver; font-size:10pt;font-family:Segoe UI Semibold;}'
...
def icon(widget, path):
    widget.setStyleSheet(
        'QPushButton{border:none;background-color:transparent;background-image:url(' + path + ');background-position:center;background-repeat:no-repeat;background-attachment:fixed;background-clip:padding;border-style:flat;border-width:0px;border-color:transparent;padding:0px;margin:0px;}')
